...
class ObjectArrayConverter implements ObjectArrayConverterContract
{
    /**
     * @var callable
     */
    ...
    public function fromArray(string $classOrInterface, array $data=[], $isFromStorage=false)
    {
        ...
        foreach ($data as $key=>$value) {
            ...
            foreach ($value as $subData) {
                $casted[$key][] = $isClass ? $this->fromArray($typeInfo['type'], (array)$subData, $isFromStorage) : $subData;
                $i++;
            }
        }
    ...
    protected function type(string $class, string $key, array $data=[])
    {
        ...
        if (is_array($data[0])) {
            return $this->type($class, $key, $data[0]);
        }
        if (is_object($data[0])) {
